...
export function DialerPanel() {
  ...
  return (
    <div className="border-t border-border px-4 py-4">
      ...
      {showSettings && (
        <div className="mb-3 rounded-lg bg-muted/50 p-2 space-y-2">
          ...
          <button
            ...
            className="w-full text-xs font-medium bg-foreground text-background rounded-md py-1.5 hover:opacity-90"
          >
            Save
          </button>
        </div>
      ...
      <Input
        ...
        className="text-center font-mono text-base mb-2"
      />
      ...
      <div className="grid grid-cols-3 gap-1.5 mb-2">
        {KEYS.flat().map((k) => (
          <button
            ...
            type="button"
            onClick={() => press(k)}
            className="aspect-square rounded-full bg-[oklch(0.96_0.04_290)] text-[var(--gold-foreground)] font-display text-lg font-semibold hover:opacity-80 active:scale-95 transition"
          >
            {k}
          </button>
        ))}
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-2 rounded-full bg-emerald-600 text-white font-medium text-sm py-2 hover:bg-emerald-700 disabled:opacity-50 transition"
        >
        ...
        <button
          onClick={backspace}
          disabled={!number}
          className="rounded-full bg-muted text-foreground p-2 hover:bg-muted/70 disabled:opacity-40"
