...
function buildScript(origin: string): string {
  ...
  try {
    ...
    if (!agentId) { console.warn('[AskJanice widget] missing ?agent= parameter'); return; }
    ...
    function init(cfg) {
      var color = (cfg && cfg.widgetColor) || '#b8893a';
      var position = (cfg && cfg.widgetPosition) || 'bottom-right';
      var sideStyle = position === 'bottom-left' ? 'left:20px;' : 'right:20px;';
      var gradient = 'linear-gradient(135deg,' + color + ',' + lighten(color, 0.2) + ')';

      var container = document.createElement('div');
      container.id = 'af-widget-container';
      container.style.cssText = 'position:fixed;bottom:20px;' + sideStyle + 'z-index:2147483647;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;';
      ...
      iframe.style.cssText = 'position:fixed;bottom:90px;' + sideStyle + 'width:380px;height:560px;max-width:calc(100vw - 40px);max-height:calc(100vh - 120px);border:none;border-radius:16px;box-shadow:0 20px 50px rgba(0,0,0,0.25);background:#fff;display:none;';
      container.appendChild(iframe);

      var bubble = document.createElement('button');
      bubble.type = 'button';
      bubble.setAttribute('aria-label','Open chat');
      bubble.style.cssText = 'width:60px;height:60px;border-radius:50%;border:none;cursor:pointer;background:' + gradient + ';color:#fff;box-shadow:0 8px 24px rgba(0,0,0,0.2);display:flex;align-items:center;justify-content:center;transition:transform .2s ease;';
    ...
    fetch(ORIGIN + '/api/public/widget/config/' + encodeURIComponent(agentId))
      .then(function(r){ return r.ok ? r.json() : null; })
      .catch(function(){ return null; })
      .then(function(cfg){ init(cfg || {}); });
  } catch (e) {
    console.error('[AskJanice widget] init error', e);
  }
})();`;
}
