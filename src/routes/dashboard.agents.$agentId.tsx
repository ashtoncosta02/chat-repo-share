...
import { speakText, transcribeAudio } from "@/lib/agent-voice.functions";
import { extractLeadFromChat } from "@/lib/agent-lead-extract.functions";
...
import { OutlookCalendarCard } from "@/components/dashboard/OutlookCalendarCard";
      ...
      <div className="flex-1 px-8 pb-4">
        <div ref={scrollRef} className="h-[42vh] overflow-y-auto space-y-4 py-4">
          ...
          {(sending || transcribing) && (
            <div className="flex items-center gap-3">
              <div className="h-9 w-9 rounded-full bg-[oklch(0.95_0.05_290)] flex items-center justify-center">
...
function MessageBubble({ msg }: { msg: Msg }) {
  ...
  return (
    <div className={`flex items-start gap-3 ${isUser ? "flex-row-reverse" : ""}`}>
      {!isUser && (
        <div className="h-9 w-9 rounded-full bg-[oklch(0.95_0.05_290)] flex items-center justify-center shrink-0">
          <Bot className="h-5 w-5 text-[var(--gold)]" />
        </div>
      )}
      <div
